...
运算符连续		ERROR
#运算符连续
运算符无效（运算符前无运算数）		ERROR
#运算符前无运算数
运算符无效（运算符后无运算数）		ERROR
#运算符后无运算数
运算符无效（小括号前无运算数，括号非第一个字符）		ERROR
#括号非第一个字符且之前无运算数 
运算符无效（小括号后无运算数，括号非最后一个字符）		ERROR
#小括号后无运算数，括号非最后一个字符*/

import java.util.Stack;
/*import java.util.readAndWriteFromFile;*/
...
public class cal {
	...
	public static boolean check(String str){
		...
		while(/*!(str.charAt(i)>=48 && str.charAt(i)<=57) && */i<str.length() ){//将表达式遍历一遍
			if(i+1>=str.length()){
				break;
			}
	...
	//转化为栈的形式并计算
	public static Float transfer(String str){    //求值
		str=str+'$';
