...
public class B1 {
    ...
    public String insetBlanks(String s) {
        String result = "";
        for (int i = 0; i < s.length(); i++) {//这个函数的作用就是使用空格分割字符串，以便后面使用分割函数使得将字符串分割成数组
            if (s.charAt(i) == '(' ||
            	s.charAt(i) == ')' ||   
            	s.charAt(i) == '+' ||
            	s.charAt(i) == '-' || 
    ...
    public void processAnOperator(Stack<Integer> operandStack, Stack<Character> operatorStack) {  //这个函数的作用就是处理栈中的两个数据，然后将栈中的两个数据运算之后将结果存储在栈中
